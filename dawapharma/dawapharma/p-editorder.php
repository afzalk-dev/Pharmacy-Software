        ...
        <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-primary">Edit Purchase Invoice </h3> </div>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="input-states">
                                    <form class="form-horizontal" method="POST"  id="createOrderForm" action="php_action/p-order.php" action="php_action/p-order.php">

                                    <?php $pid = $_GET['id']; 
                                    $sql = "SELECT * FROM purchase_invoice where pid = {$pid}";
                                    ...
                                    ?>    
                                     ...
                                     <div class="form-group">
                                            <div class="row">
                                                <label class="col-sm-2 control-label">Comapny Bill No.</label>
                                                <div class="col-sm-4">
                                                 <input type="text" class="form-control" id="clientName" name="clientName" placeholder="Company Bill No." value="<?php echo $row['compnaybill_no'] ?>" autocomplete="off" />
                                               </div>
                                                ...
                                                <select name="" id="" class="form-control">
                                                  ...
                                                  $sqlq = "SELECT brand_id,  brand_name  FROM brands WHERE brand_status = 1"; 
                                                  $run1 = mysqli_query($connect , $sqlq);
                                                  while($row3 = mysqli_fetch_assoc($run1))
                                                  {
                                                  echo '<option value="'.$row3['brand_id'].'" >'.$row3['brand_name'].'</option>';
                                                  }
                                                  ?>
                                                </select>
                                               </div>
            ...
            for($x = 1; $x < 2; $x++) { ?>
              <tr id="row<?php echo $x; ?>" class="<?php echo $arrayNumber; ?>">                
                <td style="margin-left:20px;">
                  ...
                  <select class="form-control chosen" name="productName[]" id="productName<?php echo $x; ?>" onchange="getProductData(<?php echo $x; ?>)" >
              ...
              <td style="padding-left:20px;">
                ...
                <td style="padding-left:20px;">
                  <div class="form-group">
                  <input type="number" name="disc[]" id="disc<?php echo $x; ?>" onkeyup="discAmount(<?php echo $x ?>)" autocomplete="off" class="form-control"  />
                  </div>
                </td>
                <td >                 
...
<td >
            ...
            ?>
              ...
              <input type="hidden" class="form-control" id="subTotalValue" name="subTotalValue" />
                                        ...
                                        <div class="form-group">
                                            <div class="row">
                                                <label for="discount" class="col-sm-2 control-label">Tax %</label>
                                                <div class="col-sm-4">
                                        ...
                                        <div class="form-group">
                                            <div class="row">
                                                <label for="vat" class="col-sm-2 control-label gst">Bonus</label>
                                                <div class="col-sm-4">
                                                  <!-- <input type="text" class="form-control" id="vat" name="gstn" readonly="true" />
                                                  <input type="hidden" class="form-control" id="vatValue" name="vatValue" /> -->
                                                  <input type="text" class="form-control" id="" name="bonus" autocomplete="off"  />
                                                </div>
            ...
            <div class="col-sm-4">
              <select class="form-control" name="paymentStatus" id="paymentStatus">
                <option value="">~~SELECT~~</option>
                <option value="FullPeyment">Full Payment</option>
                <option value="AdvancePeyment">Advance Payment</option>
                <option value="NoPeyment">No Payment</option>
              </select>
            </div>
        ...
        <div class="form-group submitButtonFooter">
          <div class="col-sm-offset-2 col-sm-10">
            ...
            <!-- <button type="button" class="btn btn-warning btn-flat m-b-30 m-t-30" ><a href="" style="color:white;" > Print</a></button> -->
    ...
    success:function(response) {
      ...
      var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+                
        '<td>'+
          ...
          '<select class="form-control chosen" name="productName[]" id="productName'+count+'" onchange="getProductData('+count+')" >'+
            '<option value="">~~SELECT~~</option>';
            // console.log(response);
...
function getProductData(row = null) {
  ...
  if(row) {
    ...
    if(productId == "") {
      ...
      $.ajax({
        url: 'php_action/p-fetchSelectedProduct.php',
        type: 'post',
        ...
        success:function(response) {
          ...
          $("#rate"+row).val(response.tp);
          $("#rateValue"+row).val(response.tp);

          $("#quantity"+row).val(1);
          $("#available_quantity"+row).text(response.quantity);

          var total = Number(response.tp) * 1;
          ...
          $(".chosen" + row).trigger("chosen:updated");
        } // /success
...
function discAmount(row = null) {
  if (row) {
    const discount = Number($("#disc" + row).val());
    const originalTotal = Number($("#totalValue" + row).val());

    if (discount > 0 && discount <= 100) {
      const discountedAmount = originalTotal * (1 - discount / 100);
      $("#total" + row).val(discountedAmount.toFixed(2));
      $("#totalValue" + row).val(discountedAmount.toFixed(2));

      subAmount(); // Recalculate totals
    } else {
      $("#total" + row).val(originalTotal.toFixed(2));
      $("#totalValue" + row).val(originalTotal.toFixed(2));
...
function discountFunc() {
  ...
  if (totalAmount && discount) {
    ...
    if (discount > 0 && discount <= 100) {
      var discountAmount = totalAmount * (discount / 100);  // Calculate percentage discount
      ...
      // Handle invalid discount (optional)
      $("#discount").val("");  // Clear invalid discount value
      alert("Discount must be between 0 and 100");
    }
  } else {
  ...
  // Ensure due amount calculation and display even if discount is invalid
  var paid = $("#paid").val();

  var dueAmount;
  if (paid) {
    dueAmount = totalAmount - Number(paid);  // Use original totalAmount for due calculation
  } else {
    dueAmount = totalAmount - discountAmount;  // Use discounted amount if discount is valid
  }
  $("#grandTotal").val(grandTotal.toFixed(2));
      $("#grandTotalValue").val(grandTotal.toFixed(2));
  $("#due").val(dueAmount.toFixed(2));
  $("#dueValue").val(dueAmount.toFixed(2));
}
